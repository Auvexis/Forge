# Fabric Auvexis Backend OAuth Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement the complete backend OAuth connection flow between a Fabric local profile and Auvexis Accounts, including secure per-profile storage, status, local logout, and remote revocation.

**Architecture:** Fabric keeps all OAuth tokens backend-only and profile-scoped. Auvexis Accounts remains the authority for identity, badges, and product authorization; Fabric stores only encrypted tokens plus a display-only profile snapshot. Fastify routes expose safe status/actions to the frontend and never return tokens.
...
- Modify `apps/api/src/core/server.ts`: register the new Auvexis routes with `activeProfileRuntime.activeProfileService` and `fabricHomePaths.home`.
...
All route responses must follow Fabric's existing API response shape:
...
- `POST /auvexis/account/connect/start`
  - Reads the active Fabric profile.
...
Store one file per local Fabric profile:
...
describe("Auvexis account config", () => {
  it("loads defaults and required secrets", () => {
    const config = loadAuvexisAccountConfig({
      AUVEXIS_CLIENT_ID: "fabric",
      AUVEXIS_TOKEN_ENCRYPTION_SECRET: "x".repeat(32),
    });

    assert.equal(config.clientId, "fabric");
  ...
  it("rejects missing client id and token secret", () => {
    ...
    assert.throws(
      () => loadAuvexisAccountConfig({ AUVEXIS_CLIENT_ID: "fabric" }),
...
- Return existing Fabric API response shape.
...
Run the route tests with fake service only. Do not execute a real OAuth callback until Accounts dev server and Fabric OAuth client registration are configured.
