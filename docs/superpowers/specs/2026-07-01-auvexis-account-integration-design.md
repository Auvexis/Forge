...
Connect one Auvexis Account to one local Fabric profile through OAuth 2.0 Authorization Code with PKCE. Auvexis Accounts remains authoritative for identity, badges, entitlements, and the active Fabric authorization. Fabric persists the connection securely and grants badge-based cosmetic benefits only after online backend validation.
...
- Fabric account connection, local logout, remote revocation, persistence, and reauthentication.
- Backend validation of badges before granting Fabric benefits.
...
- Fabric owns local profiles, encrypted token persistence, and the mapping from validated badges to Fabric capabilities.
- The Fabric frontend is never authoritative for tokens, badges, or capabilities.
- Local cached account data is never used for authorization.
- If Accounts cannot validate the connection online, Fabric hides Auvexis data and disables all badge-derived capabilities.
...
- One Auvexis account may have one active authorization for the Fabric OAuth client.
- Connecting that account from a new Fabric profile invalidates its previous Fabric authorization.
- The same Auvexis account may remain connected to other Auvexis products simultaneously.
- A Fabric installation maintains a local `account_id -> profile_id` index for cleanup and user experience, but Accounts enforces the security invariant.
...
Every protected Accounts request verifies that the referenced authorization remains active. Issuing a new Fabric authorization atomically replaces the previous one. This prevents an older Fabric profile from continuing to use an otherwise valid JWT until its expiration.
...
The integration is a native Fabric feature, not a plugin. It must not use the plugin credential tables or expose Auvexis-specific behavior through generic plugin contracts.

Backend responsibilities belong in a focused Auvexis account module. Frontend responsibilities belong in an Auvexis account feature. The frontend communicates only with Fabric backend endpoints.

### Per-profile persistence

Each Fabric profile stores a dedicated Auvexis connection record containing:
...
The persisted snapshot prevents needless data loss but is not returned to the frontend until Accounts validates the connection during the current Fabric session.
...
- `revocation_pending`: local data is hidden while Fabric retries an explicitly requested remote revocation.

Temporary network failures must not be interpreted as token revocation.

## OAuth Connection Flow

1. The active Fabric profile requests a connection.
2. Fabric creates state, nonce, and PKCE S256 values in backend memory with a short expiration and binds them to the initiating profile.
3. Fabric opens the system browser at the Accounts authorization endpoint.
...
6. Fabric validates state and exchanges the code through the SDK.
7. Fabric fetches the profile from Accounts and confirms the active authorization.
8. Accounts atomically invalidates the previous Fabric authorization for the same account.
9. Fabric removes any local connection for that account from another profile.
10. Fabric encrypts and stores the new tokens and profile snapshot in the initiating profile.

If Fabric restarts or the user switches profiles during authorization, the in-memory transaction expires and the user starts again. Tokens are never returned to the frontend.

## Startup and Refresh

On startup or profile activation, Fabric silently validates the connection. It refreshes tokens when needed without opening a browser. The browser is opened again only when the refresh token is invalid, expired, or revoked.
...
Fabric profiles remain independent from Auvexis Accounts. Connecting does not overwrite the Fabric profile name, avatar, or email. The linked Auvexis identity is displayed separately.

For every account or capability request:

1. Fabric refreshes the token if necessary.
2. Fabric calls Accounts from its backend.
3. Accounts validates the token and active product authorization.
4. Accounts reads only active, non-revoked entitlements.
5. Fabric maps stable badge IDs or slugs to Fabric-specific capabilities.
6. Fabric returns verified presentation data and computed capabilities to its frontend.
...
An initial Fabric capability may map the `donator` badge to special themes. Without online validation, both the linked account presentation and the special themes remain unavailable.
...
- Accounts 403 responses do not become authentication failures; Fabric reports missing scope or entitlement.
...
- Tests proving a new Fabric authorization invalidates old access and refresh tokens.
...
3. Add the native per-profile Auvexis account integration to Fabric.
4. Add Fabric badge-to-capability mappings and special themes.
