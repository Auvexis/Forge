...
Pre-release tags like `v0.1.0-alpha.10` publish:
...
```text
FABRIC_VERSION=0.1.0-alpha.10
```

Use `latest` only for stable release deployments. Use explicit tags like `0.1.0-alpha.10` for alpha and beta.
...
4. Enter the exact `package.json` version, for example `0.1.0-alpha.10`, `0.1.0-beta.1`, or `0.1.0`.
5. Wait for the tag CI to publish Docker images.
6. Deploy with:

```sh
FABRIC_VERSION=0.1.0-alpha.10 docker compose -f docker-compose.prod.yml pull
