...
describe('agent panel modal contract', () => {
  ...
  it('syncs agent execution mode between the global composer and workflow agent node', () => {
    ...
    assert.match(composer, /executionMode: 'plan'/)
    assert.match(composer, /const EXECUTION_MODE_OPTIONS[\s\S]*value: 'plan'[\s\S]*value: 'loop'/)
    assert.match(store, /selectedAgent\.value\?\.executionMode \?\? 'plan'/)
