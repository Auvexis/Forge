...
<template>
  <BaseAdvancedNode :id="props.id" :selected="props.selected" :status="props.status" :title="props.data?.name || 'Basic LLM Chain'" description="Prompt and response" :handlers="handlers" auto-organize has-target has-source :has-outgoing-connection="props.hasOutgoingConnection" bg="transparent" border-color="var(--sailor-node-border)">
