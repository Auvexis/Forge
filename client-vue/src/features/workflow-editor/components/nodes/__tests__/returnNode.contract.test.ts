...
test('return node presentation is terminal and uses return visual tokens', () => {
  ...
  assert.match(node, /var\(--sailor-node-return-icon\)/)
  assert.match(tokens, /--sailor-node-return-bg/)
  assert.match(tokens, /--sailor-node-return-icon/)
  assert.match(tokens, /--sailor-node-return-border/)
  assert.match(executionTree, /return: 'corner-down-left'/)
  assert.match(executionTree, /return: 'var\(--sailor-node-return-icon\)'/)
})
...
test('utility catalog exposes Return as a normal terminal flow node', () => {
  ...
  assert.match(returnBlock, /var\(--sailor-node-return-icon\)/)
})
