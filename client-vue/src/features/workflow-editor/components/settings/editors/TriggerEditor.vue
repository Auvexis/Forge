    ...
    <template v-if="node.data.type === 'form'">
      <div class="te-section">
        ...
        <div class="te-field">
          ...
          <button type="button" class="te-theme-open-btn" @click="isFormThemeMenuOpen = true">
            <span class="te-theme-open-preview" :style="formThemeSwatchStyle"></span>
            <span>
              <strong>{{ activeFormThemeLabel }}</strong>
        ...
        <BaseMiniMenu
          :is-open="isFormThemeMenuOpen"
          title="Form Theme"
          icon="palette"
          max-width="860px"
          max-height="88vh"
          @close="isFormThemeMenuOpen = false"
        >
          <div class="te-theme-menu">
            <div class="te-theme-preview" :style="formThemePreviewStyle">
              <div class="te-theme-preview-card" :style="formThemePreviewCardStyle">
                <span class="te-theme-preview-badge">Preview</span>
                <h3>{{ (node.data as unknown as WorkflowTrigger).formTitle || 'Contact us' }}</h3>
                <p>{{ (node.data as unknown as WorkflowTrigger).formDescription || 'This is how your public form will feel.' }}</p>
                <div class="te-theme-preview-input" :style="formThemePreviewInputStyle">Email</div>
                <div class="te-theme-preview-input" :style="formThemePreviewInputStyle">Message</div>
                <button type="button" class="te-theme-preview-button" :style="formThemePreviewButtonStyle">Submit</button>
              </div>
            </div>

            <div class="te-theme-menu-controls">
              <div class="te-theme-preset-list">
                <button
                  v-for="preset in FORM_THEME_PRESET_CARDS"
                  :key="preset.value"
                  type="button"
                  class="te-theme-preset-card"
                  :class="{ 'te-theme-preset-card--active': (formTheme.preset || 'default-floating') === preset.value }"
                  @click="applyFormThemePreset(preset.value)"
                >
                  <span class="te-theme-preset-chip" :style="{ background: preset.swatch }"></span>
                  <span>
          ...
          <template #footer>
            <BaseButton variant="ghost" @click="isFormThemeMenuOpen = false">Close</BaseButton>
          </template>
        </BaseMiniMenu>
...
const isFormThemeMenuOpen = ref(false)
onMounted(() => {
...
const formThemePresetDefaults: Record<NonNullable<FormTheme['preset']>, FormTheme> = {
  'default-floating': {
...
const activeFormTheme = computed<FormTheme>(() => {
  const preset = formTheme.value.preset ?? 'default-floating'
  const defaults = formThemePresetDefaults[preset]
  return {
    ...defaults,
    ...formTheme.value,
    background: { ...defaults.background, ...formTheme.value.background },
    container: { ...defaults.container, ...formTheme.value.container },
    button: { ...defaults.button, ...formTheme.value.button },
    fields: { ...defaults.fields, ...formTheme.value.fields },
  }
})

const activeFormThemeLabel = computed(() => {
  const preset = formTheme.value.preset ?? 'default-floating'
  return FORM_THEME_PRESET_CARDS.find((item) => item.value === preset)?.label ?? 'Default Floating'
})

const formThemeBackground = computed(() => {
  const background = activeFormTheme.value.background
...
const formThemeSwatchStyle = computed(() => ({ background: formThemeBackground.value }))
const formThemePreviewStyle = computed(() => ({ background: formThemeBackground.value }))
const formThemePreviewCardStyle = computed(() => ({
  background: activeFormTheme.value.container?.backgroundColor,
  borderColor: activeFormTheme.value.container?.borderColor,
  borderRadius: `${activeFormTheme.value.container?.radius ?? 12}px`,
}))
const formThemePreviewInputStyle = computed(() => ({
  background: activeFormTheme.value.fields?.backgroundColor,
  borderColor: activeFormTheme.value.fields?.borderColor,
  borderRadius: `${activeFormTheme.value.fields?.radius ?? 8}px`,
  color: activeFormTheme.value.fields?.textColor,
}))
const formThemePreviewButtonStyle = computed(() => ({
  width: activeFormTheme.value.button?.width === 'full' ? '100%' : 'fit-content',
  background: activeFormTheme.value.button?.backgroundColor,
  borderColor: activeFormTheme.value.button?.borderColor,
  borderRadius: activeFormTheme.value.button?.shape === 'pill'
    ? '999px'
    : activeFormTheme.value.button?.shape === 'square'
      ? '0'
      : '8px',
  color: activeFormTheme.value.button?.textColor,
}))
...
function applyFormThemePreset(preset: NonNullable<FormTheme['preset']>) {
  props.updateNodeData({ formTheme: formThemePresetDefaults[preset] })
}
...
.te-theme-open-btn:hover {
  border-color: var(--nod8-accent);
}

.te-theme-open-preview {
  width: 44px;
  height: 34px;
  flex-shrink: 0;
  border: 1px solid var(--nod8-border);
...
.te-theme-open-btn strong,
.te-theme-preset-card strong {
  display: block;
  font-size: 13px;
  font-weight: 700;
}

.te-theme-open-btn small,
.te-theme-preset-card small {
  display: block;
  margin-top: 2px;
...
.te-theme-preview-card h3 {
  margin: 0;
  color: var(--nod8-text-primary);
  font-size: 20px;
}

.te-theme-preview-card p {
  margin: 0;
  color: var(--nod8-text-secondary);
  font-size: 12px;
  line-height: 1.45;
...
.te-theme-preview-input {
  padding: 9px 10px;
  border: 1px solid;
  font-size: 12px;
}

.te-theme-preview-button {
  padding: 10px 14px;
  border: 1px solid;
  font-size: 12px;
  font-weight: 700;
...
.te-theme-menu-controls {
  display: flex;
  flex-direction: column;
  gap: 12px;
}

.te-theme-preset-list {
  display: grid;
  grid-template-columns: repeat(3, minmax(0, 1fr));
  gap: 8px;
}

.te-theme-preset-card {
  min-height: 92px;
  display: flex;
  flex-direction: column;
  gap: 8px;
