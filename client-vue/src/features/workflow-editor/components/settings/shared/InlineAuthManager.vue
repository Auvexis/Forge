...
<template>
  <div v-if="pluginStatus && pluginStatus.auth_type !== 'none'" class="inline-auth-manager surface rounded-md mb-4 border border-nod8-border overflow-hidden">
    <!-- Header Summary -->
    <div 
      class="flex-between p-3 cursor-pointer select-none hover:bg-[var(--nod8-bg-elevated)] transition-colors"
      style="background: var(--nod8-bg-surface);"
      @click="toggleOpen"
    >
      <div class="flex items-center gap-2">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--nod8-text-muted)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <div v-if="isOpen" class="p-3 border-t border-nod8-border" style="background: var(--nod8-bg-elevated);">
