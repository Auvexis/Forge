...
function startServer(status = 201): Promise<{ url: string; close: () => Promise<void> }> {
  const server = http.createServer((req, res) => {
    assert.equal(req.headers.authorization, "Bearer secret-token");
    res.statusCode = status;
    res.setHeader("Content-Type", "application/json");
    res.end(JSON.stringify(status >= 400 ? { message: "bad" } : { id: "lead_1" }));
  });
...
describe("PluginMethodPlanRunner", () => {
  it("runs request, response mapper and code block with per-node trace", async () => {
    ...
    const runner = new PluginMethodPlanRunner({ now: () => "2026-05-20T00:00:00.000Z" });
  ...
  it("runs only the selected if branch and return controls final output", async () => {
    ...
    assert.equal(result.trace.some((event) => event.nodeId === "return_no"), false);
  });
  ...
  it("maps configured errors as failed node trace", async () => {
    ...
    try {
      ...
      assert.equal(result.trace.some((event) => event.type === "node:failed"), true);
    } finally {
      await server.close();
    }
  });
  ...
  it("runs switch default and aggregates forEach outputs", async () => {
    ...
    assert.equal(result.trace.some((event) => event.nodeId === "return_single"), false);
  });
  ...
  it("try/catch catches code block errors and runs catch steps", async () => {
    ...
    assert.equal(result.trace.some((event) => event.type === "node:failed" && event.nodeId === "code_throw"), true);
  });
});
