...
const LIST_TABLES_RPC_SQL = `create or replace function sailor_list_tables(target_schema text default 'public')
...
export function createSupabaseMethods() {
  return {
    ...
    async listTables(params: ListTablesParams, context?: PluginContext) {
      ...
      const { data, error } = await client.rpc("sailor_list_tables", { target_schema: schema });
