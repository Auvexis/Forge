...
- [x] Task 6: Fix VariableTree plugin icons to use theme-aware icon resolution.
- [x] Task 7: Add predefined plugin categories to SDK schema/types, update CLI template/prompts, and update server dependency.
- [x] Task 8: Group Add Node Panel plugins by category with category filter and collapse/expand controls.
- [x] Task 9: Run focused tests/build checks and record publish/install result.
...
Verification:
- Published `@auvexis/sailor-sdk@1.3.0` to npm and installed it in `sailor/server` and `sailor-cli`.
- SDK: `npm test`, `npm run typecheck`, `npm run build`.
- CLI: `npm test`.
- Sailor server: plugin loader/manifest preview tests and `npx tsc --noEmit`.
- Sailor client: AddNodePanel contract test and `npm run type-check`.
