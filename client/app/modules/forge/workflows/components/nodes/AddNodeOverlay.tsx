...
import {
  X, Search, ChevronLeft, Workflow, Blocks, Loader2,
  Code2, GitBranch, Repeat, Layers, Globe, Zap,
} from "lucide-react";
...
const LOGIC_NODES: LogicNodeDef[] = [
  { type: "code",        label: "Code Block",    description: "Execute custom JavaScript in a sandbox",   icon: Code2,     color: "text-amber-400",  bg: "bg-amber-500/10"  },
  { type: "if",          label: "If / Else",     description: "Branch the flow based on a condition",     icon: GitBranch, color: "text-violet-400", bg: "bg-violet-500/10" },
  { type: "loop",        label: "Loop / ForEach", description: "Iterate over a collection per item",      icon: Repeat,    color: "text-cyan-400",   bg: "bg-cyan-500/10"   },
  { type: "subworkflow", label: "Sub-Workflow",  description: "Call another workflow as a nested step",   icon: Layers,    color: "text-rose-400",   bg: "bg-rose-500/10"   },
  { type: "http",        label: "HTTP Request",  description: "Make an HTTP request to any external API", icon: Globe,     color: "text-orange-400", bg: "bg-orange-500/10" },
  { type: "event",       label: "Emit Event",    description: "Publish an event to trigger other flows",  icon: Zap,       color: "text-yellow-400", bg: "bg-yellow-500/10" },
];
...
export const AddNodeOverlay = ({ onAddNode, onAddLogicNode, onClose }: Props) => {
  ...
  const goBack = () => {
    if (view === "actions") { setView("plugins"); setSelectedPluginId(null); setSearch(""); }
    else if (view === "plugins") { setView("categories"); setSearch(""); }
  };

  const selectPlugin = (id: string) => { setSelectedPluginId(id); setView("actions"); setSearch(""); };

  const searchPlaceholder =
    view === "categories" ? "Search components..."
    : view === "plugins"  ? "Search integrations..."
    : "Search actions...";
