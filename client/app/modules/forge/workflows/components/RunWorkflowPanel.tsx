    ...
    <div className="absolute top-20 right-4 w-[400px] z-[70] bg-card/95 backdrop-blur-xl border border-border shadow-2xl rounded-2xl flex flex-col overflow-hidden animate-in slide-in-from-right-10 duration-300 max-h-[calc(100%-110px)]">
      <div className="flex items-center justify-between p-4 border-b border-border bg-primary/10 shrink-0">
        <div className="flex flex-col">
          <h3 className="font-black text-micro uppercase tracking-widest text-primary">Run Configuration</h3>
      ...
      <div className="flex-1 overflow-y-auto p-5 custom-scrollbar flex flex-col gap-6">
        <div className="flex flex-col gap-1.5 p-4 rounded-xl bg-primary/5 border border-primary/10">
           <div className="flex items-center gap-2 text-primary">
              ...
              <span className="text-tiny font-black uppercase tracking-widest">Trigger Info</span>
           </div>
                 ...
                 return (
                   <div key={key} className="flex flex-col gap-2">
                      <div className="flex justify-between items-center px-1">
                         ...
                         <span className="text-micro bg-accent/30 px-1.5 py-0.5 rounded text-muted-foreground uppercase font-black">
                           {field.type}
                         </span>
                      </div>
                      {isFile ? (
                        <Input 
                          ...
                          className="bg-accent/10 border-border/50 h-10 pt-1.5 font-semibold text-sm cursor-pointer file:bg-primary file:text-primary-foreground file:border-0 file:rounded-md file:px-2 file:py-1 file:text-mini file:font-bold file:uppercase file:mr-4 file:hover:bg-primary/80 transition-all"
                        />
                      ) : (
                        <Input 
