 ...
 *
 * @param properties   The `properties` map from a schema object
 * @param prefix       The dot-path prefix so far, e.g. "steps.n.output"
 * @param labelPrefix  The human-readable label prefix, e.g. "Download"
 * @param nodeId       ID of the source node
 * @param nodeName     Display name of the source node
...
function walkProperties(
  ...
  for (const [key, prop] of Object.entries(properties)) {
    ...
    if (prop.type === "object" && prop.properties) {
      ...
      // Recurse into array item's properties with [0] index notation
      paths.push(
        ...walkProperties(
          prop.items.properties,
          `${fullPath}[0]`,
 ...
 * @param schema    The method's responseSchema (from manifest)
 */
export function resolveSchemaTree(
  ...
  // Array response: inspect items.properties
  ...
  // Object response: inspect properties directly
  if (schema.type === "object" && schema.properties) {
    return walkProperties(schema.properties, prefix, "", nodeId, nodeName);
  }

  // Scalar / no properties — offer the whole output
  return [
    {
      path: prefix,
      label: "Output",
...
/**
 * Resolve all trigger variable paths from the trigger schema.
