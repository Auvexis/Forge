...
```bash
FABRIC_VERSION=0.1.0-alpha.13 docker compose -f docker-compose.prod.yml up -d
```

PowerShell:

```powershell
$env:FABRIC_VERSION = "0.1.0-alpha.13"
...
```bash
FABRIC_VERSION=0.1.0-alpha.13 FABRIC_PUBLIC_URL=https://example.ngrok-free.app docker compose -f docker-compose.prod.yml up -d
```
...
```text
ghcr.io/auvexis/fabric-api:0.1.0-alpha.13
ghcr.io/auvexis/fabric-client:0.1.0-alpha.13
ghcr.io/auvexis/fabric-gateway:0.1.0-alpha.13
```
