...
function createCanvasItems(
    ...
    .map((element, index) => {
      ...
      return {
        ...
        data: {
          ...
          accent: 'var(--fabric-page-blueprint-workbench-blue400)',
          showFooter: true,
  ...
  const utilityItems = utilityNodes.map((node, index) => {
    ...
    return {
      ...
      data: {
        ...
        accent: node.accent ?? definition?.accent ?? 'var(--fabric-page-blueprint-workbench-accent)',
        showFooter: false,
        fields: [],
        utilityNode,
      },
  ...
  const componentItems = document.value.nodes
    ...
    .map((node, index) => ({
      ...
      data: {
        ...
        accent: node.accent ?? 'var(--fabric-page-blueprint-workbench-accent)',
        showFooter: false,
        fields: [],
  ...
  return [{
    ...
    data: {
      ...
      accent: 'var(--fabric-page-blueprint-workbench-text-muted)',
      showFooter: false,
      fields: [{
        id: 'hint',
...
function buildGroupFrames(groups: Array<{ id: string; name: string; kind: 'group' | 'component'; color?: string; nodeIds: string[] }>, items: PageBlueprintCanvasItem[]) {
  ...
  return groups.flatMap((group) => {
    ...
    return [{
      ...
      name: group.name,
      kind: group.kind,
      color: group.color ?? 'var(--fabric-page-blueprint-workbench-accent)',
      nodeIds: group.nodeIds,
