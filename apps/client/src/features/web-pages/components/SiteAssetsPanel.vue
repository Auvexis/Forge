<template>
  ...
  >
    ...
    <div
      v-if="uploadingAssets.length"
      class="web-page-assets-panel__upload-preview"
    >
    ...
    <div v-if="assetFiles.length" class="web-page-assets-panel__grid">
      ...
      >
        ...
        <button
          type="button"
          title="Delete asset"
          @click="$emit('delete-file', asset.path)"
        >
...
const assetFiles = computed(() =>
  (props.site?.files ?? []).filter(
    (file) => file.kind === "asset" || file.path.startsWith("assets/"),
  ),
);
