<template>
  <aside
    ref="panelRef"
    class="web-page-toolbox"
    aria-label="HTML element toolbox"
  >
    ...
    <RenderPortal>
      ...
      >
        <div
          class="web-page-toolbox-drag-preview__body"
        ...
        >
          ...
          <div class="web-page-toolbox-drag-preview__label">
            {{ dragPreview.label }}
          </div>
          <div class="web-page-toolbox-drag-preview__subtitle">
...
const toolboxItemsById = computed(
  () =>
    new Map(
      toolboxSections.flatMap((section) =>
...
function onDragStart(event: DragEvent, item: ToolboxItem) {
  ...
  if (item.kind === "page")
    event.dataTransfer?.setData(
      "application/x-fabric-page",
      JSON.stringify({ type: "page" }),
    );
  else
    event.dataTransfer?.setData(
      "application/x-fabric-page-block",
      JSON.stringify({ tag: item.tag, preset: item.id }),
    );
...
function rememberItem(item: ToolboxItem) {
  recentItemIds.value = [
    item.id,
    ...recentItemIds.value.filter((id) => id !== item.id),
  ]
    .filter((id) => toolboxItemsById.value.has(id))
    .slice(0, RECENT_ITEMS_LIMIT);
  localStorage.setItem(
    RECENT_ITEMS_STORAGE_KEY,
    JSON.stringify(recentItemIds.value),
  );
}
...
function loadRecentItems() {
  ...
  try {
    ...
    recentItemIds.value = ids
      .filter(
        (id): id is string =>
          typeof id === "string" && toolboxItemsById.value.has(id),
      )
      .slice(0, RECENT_ITEMS_LIMIT);
  } catch {
    recentItemIds.value = [];
...
function moveDragPreview(point: { x: number; y: number }) {
  ...
  const lateralPull = Math.max(
    -58,
    Math.min(58, dx * 6 + dragPreviewVelocity.value.x * 0.65),
  );
