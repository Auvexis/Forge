...
describe('Page Actions architecture', () => {
  ...
  it('keeps Code and Blueprint document modes out of the old modal and side chrome', () => {
    ...
    expect(panel).toMatch(/Bind Return Output/)
    expect(panel).toMatch(/usePageActionBindingsStore/)
    expect(panel).toMatch(/usePageEditorStore/)
    expect(panel).toMatch(/bindingsStore\.bindOutputToElement/)
    expect(panel).toMatch(/editorStore\.patchBlock/)
