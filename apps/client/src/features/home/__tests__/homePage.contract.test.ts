...
describe('home page contract', () => {
  ...
  it('renders a flat software welcome page without IDE chrome', () => {
    ...
    assert.match(welcome, /home-welcome__sections/)
    assert.match(welcome, /home-welcome__media-slot/)
    assert.match(welcome, /home-welcome__editor-media-slot/)
    assert.match(welcome, /<div class="home-welcome__media-slot" aria-hidden="true" \/>/)
    assert.match(welcome, /<div class="home-welcome__editor-media-slot" aria-hidden="true" \/>/)
    assert.match(welcome, /home-welcome__quick-actions/)
    assert.match(welcome, /home-welcome__editors/)
    assert.match(welcome, /\.home-welcome\s*\{[\s\S]*width: 100%;[\s\S]*flex: 1;/)
    assert.match(welcome, /\.home-welcome__sections\s*\{[\s\S]*width: 100%;[\s\S]*padding: var\(--fabric-space-8\);/)
    assert.match(welcome, /\.home-welcome__intro\s*\{[\s\S]*width: 100%;/)
    assert.match(welcome, /\.home-welcome__intro\s*\{[\s\S]*min-height: 420px;/)
    assert.match(welcome, /\.home-welcome__intro\s*\{[\s\S]*padding: 0 0 var\(--fabric-space-10\);/)
    assert.match(welcome, /\.home-welcome__editors\s*\{[\s\S]*width: 100%;/)
    assert.match(welcome, /\.home-welcome__editors\s*\{[\s\S]*padding: var\(--fabric-space-10\) 0 0;/)
    assert.doesNotMatch(welcome, /width: min\(980px, 100%\)/)
    assert.doesNotMatch(welcome, /showMediaSlots/)
