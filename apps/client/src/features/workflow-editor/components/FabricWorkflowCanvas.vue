...
function handleCanvasContextMenu(event: MouseEvent) {
  ...
  if (target?.closest(
    '[data-base-canvas-item-id], [data-workflow-handle-id], .fabric-workflow-edge, .fabric-edge-toolbar, .add-node-picker-overlay__window, button, input, textarea, select',
  )) return
...
function handleGlobalAddNodeDrop(event: DragEvent) {
  const raw = event.dataTransfer?.getData('application/x-fabric-add-node')
  if (!raw) return
...
defineExpose({
  ...
  selectNode: (nodeId: string) => workflowBaseCanvasRef.value?.selectNode(nodeId),
  focusNode: (nodeId: string) => workflowBaseCanvasRef.value?.focusNode(nodeId),
  highlightNode: (nodeId: string | null) => workflowBaseCanvasRef.value?.highlightNode(nodeId),
...
<template>
  <div
    class="fabric-workflow-canvas fabric-fill"
    @dragover.prevent
...
.canvas-empty-step:focus-visible .canvas-empty-step__box {
  outline: 2px solid var(--fabric-node-selected);
  outline-offset: 3px;
}
</style>
