...
test('chat session panel renders animated agent tool status rows', () => {
  ...
  assert.doesNotMatch(source, /detectChatLocale/)
  assert.match(source, /Retrying tool/)
  assert.match(source, /Retrying \$\{tool\}\./)
  assert.doesNotMatch(source, /Vou usar|Estou executando|Usei|Perfect, I will use|I am running|Done, I used/)
  assert.match(source, /chat-session-panel__tool-status/)
  assert.match(source, /chat-session-panel__tool-status--running/)
  assert.match(source, /chat-session-panel__tool-status--retrying/)
...
test('chat session panel renders user and assistant messages', () => {
  ...
  assert.doesNotMatch(source, /message\.role === 'user' \? 'user' : 'bot'/)
  assert.match(source, /messageDisplayName/)
  assert.match(source, /messageAvatar/)
  assert.match(source, /formatMessageTime/)
