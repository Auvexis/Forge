...
describe('workflow BaseCanvas shell contract', () => {
  ...
  it('renders the shared BaseCanvas without changing the shared implementation', () => {
    ...
    assert.match(source, /background-color="var\(--fabric-base-canvas-bg\)"/)
    assert.match(source, /pattern-color="var\(--fabric-base-canvas-grid\)"/)
  ...
  it('renders current workflow node components with workflow graph props', () => {
    ...
    assert.match(source, /:selected="canvasSelection\.includes\(item\.id\) \|\| highlightedNodeId === item\.id"/)
    assert.doesNotMatch(host, /drop-shadow/)
