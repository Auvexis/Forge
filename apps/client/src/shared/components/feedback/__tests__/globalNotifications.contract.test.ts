...
describe('global notifications UI contract', () => {
  it('provides a reusable BaseButton bell trigger with unread badge and UI-store toggle', () => {
    ...
    assert.match(trigger, /background:\s*var\(--fabric-bg-inverse\)/)
    assert.match(trigger, /color:\s*var\(--fabric-text-inverse\)/)
  })
  ...
  it('marks only selected notifications read and supports detail metadata, delete, and internal action navigation', () => {
    ...
    assert.doesNotMatch(list, /border-color:\s*var\(--fabric-border-brand\)/)
  ...
  it('uses theme tokens and reduced-motion transition classes', () => {
    ...
    assert.match(panel, /border-radius:\s*0 0 var\(--fabric-radius-sm\) var\(--fabric-radius-sm\)/)
    assert.match(panel, /background:\s*var\(--fabric-bg-surface\)/)
    assert.match(panel, /min-height:\s*min\(420px,\s*calc\(100vh - var\(--fabric-space-4\)\)\)/)
    ...
    assert.match(panel, /var\(--fabric-/)
    assert.match(trigger, /var\(--fabric-/)
  ...
  it('keeps notification detail compact inside the panel tab', () => {
    ...
    assert.match(detail, /\.notification-detail__meta\s*{[^}]*gap:\s*var\(--fabric-space-2\)/s)
    assert.match(detail, /font-size:\s*var\(--fabric-text-lg\)/)
    assert.doesNotMatch(detail, /font-size:\s*var\(--fabric-text-xl\)/)
  })
