...
describe('theme runtime contract', () => {
  ...
  it('defines VS Code-style dot tokens for shared component surfaces', () => {
    ...
    for (const tokenName of [
      ...
      'suiteNav.text.muted',
      'suiteNav.label.text',
      'suiteNav.section.text',
      'desktopWindow.radius',
      'desktopWindow.border',
      'appTopbar.windowControl.bg',
      'appTopbar.windowControl.text',
      'appTopbar.windowControl.hover.bg',
      'appTopbar.windowControl.hover.text',
      'appTopbar.windowControl.close.hover.bg',
      'appTopbar.windowControl.close.hover.text',
      'appTopbar.windowControl.radius',
      'executionPanel.bg',
    ]) {
  ...
  it('keeps the dark theme neutral instead of warm brown', () => {
    ...
    assert.equal(darkTheme.tokens['bg.base'], '#0B0B0B')
    assert.equal(darkTheme.tokens['bg.chrome'], '#141414')
    assert.equal(darkTheme.tokens['bg.canvas'], '#0B0B0B')
    assert.doesNotMatch(
      darkThemeSource,
