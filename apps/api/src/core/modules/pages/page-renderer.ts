...
function renderActionRuntime(page: PublishedPage, options: RenderOptions = {}): string {
  return [
    ...
    `    return value;`,
    `  }`,
    `  function evaluateBlueprintExpression(template, result) {`,
    `    const expression = normalizeBlueprintExpression(template);`,
    `    if (!expression.trim()) return undefined;`,
    `    const direct = resolveBlueprintExpressionPath(expression, result);`,
    `    if (direct !== undefined) return direct;`,
    `    const jsExpression = expression.replace(/\\b(?:utility|blueprint-element):[A-Za-z0-9:_-]+(?:\\.[A-Za-z0-9:_\\-[\\]]+)+(?:\\.(?:[A-Za-z_$][\\w$-]*|\\d+))*/g, (path) => JSON.stringify(resolveBlueprintExpressionPath(path, result)));`,
    `    try { return Function('"use strict"; return (' + jsExpression + ')')(); } catch { return undefined; }`,
    `  }`,
    `  function normalizeBlueprintExpression(template) {`,
    `    const trimmed = String(template || "").trim();`,
    `    const single = trimmed.match(/^\\{\\{\\s*(.+?)\\s*\\}\\}$/);`,
    `    if (single) return single[1].trim();`,
    `    return trimmed.replace(/\\{\\{\\s*((?:utility|blueprint-element):[^}]+?)\\s*\\}\\}/g, (_, path) => path.trim());`,
    `  }`,
    `  function resolveBlueprintExpressionPath(path, result) {`,
    `    const match = String(path || "").match(/^utility:run-workflow:[^.]+\\.return(?::([^\\.]+))?(?:\\[\\])?(?:\\.(.*))?$/);`,
    `    if (!match) return undefined;`,
    `    const segments = [match[1], ...(match[2] ? match[2].split(".") : [])].filter(Boolean);`,
    `    return resolveResultPath(result, segments.join("."));`,
    `  }`,
    ...
    `      return;`,
    `    }`,
    `    if (target.property === "src" || target.property === "alt" || target.property === "class" || target.property === "id") {`,
    `      element.setAttribute(target.property, text);`,
    `      return;`,
    `    }`,
    ...
    `    if (target.property === "src" || target.property === "alt" || target.property === "class" || target.property === "id") { element.setAttribute(target.property, text); return; }`,
    `    element.textContent = text;`,
    `  }`,
    `  function applyOutputBindings(actionId, result) {`,
    `    for (const binding of outputBindings[actionId] || []) {`,
    `      if (String(binding.resultPath || "") === "item" || String(binding.resultPath || "").startsWith("item.")) continue;`,
    `      const value = binding.expression ? evaluateBlueprintExpression(binding.expression, result) : resolveResultPath(result, binding.resultPath);`,
    ...
    `    } catch (error) {`,
    `      console.error("[Fabric Pages] Action failed", error);`,
    `    } finally {`,
    `      pendingActionId = "";`,
    `    }`,
    `  }`,
