...
function renderActionRuntime(page: PublishedPage, options: RenderOptions = {}): string {
  return [
    ...
    `    const jsExpression = expression.replace(/utility:run-workflow:[A-Za-z0-9:_-]+\\.return(?::[A-Za-z0-9_-]+)?(?:\\.(?:[A-Za-z_$][\\w$-]*|\\d+))*/g, (path) => JSON.stringify(resolveBlueprintExpressionPath(path, result)));`,
    ...
    `    return trimmed.replace(/\\{\\{\\s*(utility:run-workflow:[^}]+?)\\s*\\}\\}/g, (_, path) => path.trim());`,
    `  }`,
