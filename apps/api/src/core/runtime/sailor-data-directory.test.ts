...
import {
  prepareSailorDataDirectory,
  resolveLegacyDataDir,
} from "./sailor-data-directory.ts";

describe("prepareSailorDataDirectory", () => {
  it("copies legacy config/data databases into an empty SAILOR_HOME data dir", () => {
    const temp = fs.mkdtempSync(path.join(os.tmpdir(), "sailor-data-"));
    ...
    const result = prepareSailorDataDirectory({ dataDir, legacyDataDir: legacyDir });
  ...
  it("does not overwrite existing SAILOR_HOME data", () => {
    const temp = fs.mkdtempSync(path.join(os.tmpdir(), "sailor-data-"));
    const dataDir = path.join(temp, "home", "data");
    const legacyDir = path.join(temp, "config", "data");
