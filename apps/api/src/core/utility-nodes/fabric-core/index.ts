...
import { fabricCoreUtilityNodePack } from "./manifest.ts";

export interface FabricCoreUtilityNode {
...
export const fabricCoreUtilityNodes: FabricCoreUtilityNode[] = handlers.map((handler) => {
  const manifest = fabricCoreUtilityNodePack.nodes[handler.type as keyof typeof fabricCoreUtilityNodePack.nodes];
  if (!manifest) {
    throw new Error(`Missing Fabric Core utility node manifest for "${handler.type}"`);
  }
