...
describe("pinecone vector store plugin", () => {
  it("exports the generic vector store methods", () => {
    assert.equal(plugin.id, "fabric-pinecone");
  ...
  it("converts Fabric metadata filters to Pinecone flat metadata filters", () => {
  ...
  it("rejects unsupported Fabric metadata filter operators", () => {
    assert.throws(() => toPineconeFilter({
      "data.score": { "$between": [600, 900] },
    }), /Unsupported Fabric metadata filter operator "\$between"/);
  });
