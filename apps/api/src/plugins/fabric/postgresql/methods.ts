...
async function ensureAgentMemoryTable(db: Queryable) {
  await db.query(`
    create table if not exists fabric_agent_memories (
      id text primary key,
  ...
  `);
  await db.query(`
    create index if not exists fabric_agent_memories_lookup_idx
      on fabric_agent_memories (profile_id, namespace, updated_at desc)
  `);
}
...
export async function putAgentMemoryWithDb(db: Queryable, params: AgentMemoryPutParams) {
  ...
  await db.query(
    `
      insert into fabric_agent_memories
