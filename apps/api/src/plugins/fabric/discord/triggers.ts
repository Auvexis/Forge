import type { PluginTriggerHooks, TriggerRegistrationContext } from "@auvexis/fabric-sdk";
  ...
  return {
    async setup(ctx) {
      ...
      socket.onerror = (event) => {
        console.error(`[FABRIC | DISCORD TRIGGER]: Gateway error for ${key}`, event);
      };
      socket.onclose = () => {
...
async function forwardEvent(webhookUrl: string, payload: MessagingTriggerPayload): Promise<void> {
  ...
  if (!response.ok) {
    const text = await response.text();
    console.error(
      `[FABRIC | DISCORD TRIGGER]: Failed to forward event to Fabric (${response.status}): ${text || response.statusText}`,
    );
  }
}
