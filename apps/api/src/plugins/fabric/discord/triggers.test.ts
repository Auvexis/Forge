...
describe("discord triggers", () => {
  ...
  it("connects to Discord Gateway and forwards normalized message events", async () => {
    ...
    try {
      ...
      await trigger.setup({
        webhookUrl: "https://fabric.test/plugin-events/wf/trigger/discord/onMessage",
      ...
      const forwarded = fetchCalls.find((call) => call.url === "https://fabric.test/plugin-events/wf/trigger/discord/onMessage");
      assert.ok(forwarded);
